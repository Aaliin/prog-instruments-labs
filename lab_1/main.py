...
def actions_for_generating_keys(symmetric: Symmetric, asymmetric: Asymmetric,
                                settings: dict) -> None:
    """
...
def actions_to_encrypt_symmetric_key(symmetric: Symmetric, asymmetric: Asymmetric,
                                     settings: dict) -> None:
    """
    ...
    if sym_key is None:
        ...
        return
    to_write = asymmetric.process_symmetric_key(
        sym_key, settings["public_key"], "encrypt")
    Support.write_bytes_to_txt(to_write, settings["encrypted_sym"])
    print(f"The symmetric key has been successfully encrypted")


def actions_to_decrypt_symmetric_key(asymmetric: Asymmetric, 
                                     settings: dict) -> None:
    """
    ...
    sym_key = asymmetric.process_symmetric_key(
        asym_key, settings["private_key"], "decrypt")
...
def main():
    ...
    group.add_argument("-encr_k", "--encryption_key",
                       help="Encrypted of sym key", action="store_true")
    group.add_argument("-decr_k", "--decription_key",
                       help="Decrypted of sym key", action="store_true")
    group.add_argument("-encr", "--encryption",
                       help="Encrypted of text", action="store_true")
    group.add_argument("-decr", "--decryption",
                       help="Decrypted of text", action="store_true")
    group.add_argument("-clear", "--clear", 
                       help="Clear all files without original text file", 
                       action="store_true")
    ...
    match args:
        ...
        case args if args.clear:
            clean_text_files([settings["decrypted_sym"], settings["decrypted_text"],
                              settings["encrypted_text"], settings["private_key"],
                              settings["public_key"], settings["sym_key"]])
